                    ...
                    <div class="instructor_pricing_widget">
                        <div class="d-flex alogn-items-center justify-content-center m-2">
                            ...
                            <div class="card-body">
                                ...
                                <select id="currencySelector" data-course-currancy="{{ $course->currancy->currency }}"
                                    class="form-control w-150px">
                                    <option selected disabled>Select Currency ...</option>
                                    <option>None</option>
                                    @foreach ($currency as $item)

                                    @if($course->currancy->currency != $item->currency)
                                    <option @selected($item->currency ? false :true)
                                        value="{{$item->id}}"
                                        data-cuurancy-amount="{{ $item->amount }}"
                                        data-cuurancy-exchange_rate="{{ $item->exchange_rate }}"
                                        data-currancy-name="{{ $item->currency }}">
                                        {{$item->currency }}
                                    </option>
                                    @endif
                        ...
                        <div class="price">Price
                            <label><s id="originalPrice" class="t_price price text-danger">${{ $price }}</s></label>
                            <label id="discountedPrice" class="t_discount_price text-success">${{ $total_price
                                }}</label>
                        </div>
...
<script>
    $(document).ready(function () {
// Original price in the default currency
    let originalPrice = parseFloat({{ $price }});
    let discount = $(".chapter_discount");
    let discountedPrice = parseFloat({{ $total_price }});
    ...
    // Retrieve exchange rate and currency name
    let currencyAmount = parseFloat(selectedOption.data('cuurancy-amount')) || 1; // Default to 1 if undefined
    let exchangeRate = parseFloat(selectedOption.data('cuurancy-exchange_rate')) || 1; // Default to 1 if undefined
    let currencyName = selectedOption.data('currancy-name');
    ...
    function setCurrencyChoice(exchangeRate) {
    localStorage.setItem("selectedCurrency", exchangeRate);
    }
if ("{{ $course->currancy->currency }}" === "USD") {
    switch (currencyName) {
        case "USD":
            exchangeRatePrice = (originalPrice / exchangeRate).toFixed(2);
            break;
        case "EGP":
            exchangeRatePrice = (originalPrice * exchangeRate).toFixed(2);
            break;
        case "None":
            exchangeRatePrice = originalPrice.toFixed(2); // Use the original price
            break;
        default:
            exchangeRatePrice = (originalPrice * currencyAmount).toFixed(2);
            currencyName = "{{ $course->currancy->currency }}";
            break;
    }
} else {
    switch (currencyName) {
        case "USD":
            currencyEchangeRate = localStorage.getItem("selectedCurrency", exchangeRate);
            setCurrencyChoice(exchangeRate);
            exchangeRatePrice = ((originalPrice / {{ $course->currancy->amount }} ) ).toFixed(2);
            console.log(`"{{ $course->currancy->currency }}: ${exchangeRate}"`);
            break;
        case "EGP":
            console.log(exchangeRatePrice);
            currencyEchangeRate = localStorage.getItem("selectedCurrency", exchangeRate);
            setCurrencyChoice(exchangeRate);
            exchangeRatePrice = (originalPrice * currencyEchangeRate).toFixed(2);
...
// let selectedCurrency = event.target.value; // Get selected currency

console.log(`Local Storage Added ${exchangeRate}`);

console.log(`Change ${currencyName}: ${exchangeRatePrice}`);
    // Append the updated price to the `.price` element
    let convertedPriceElement = `<label id="discountedPrice" class="t_discount_price text-success">${exchangeRatePrice} ${currencyName}</label>`;

    // Clear any previous appended prices and add the new one
    $('.price').html(convertedPriceElement);

    // Log the conversion
    console.log(`Updated Price (${currencyName}): ${exchangeRatePrice}`);
    });
});
</script>
<a class="scrollToHome" href="#"><i class="flaticon-up-arrow-1"></i></a>
