...
<button type="button" class="btn btn-primary my-3" data-bs-toggle="modal" data-bs-target="#AddmodalCenter">
    Add New Promo Code
</button>
...
<div class="modal fade" id="AddmodalCenter" tabindex="-1" aria-hidden="true" style="display: none;">
    ...
    <div class="col-md-12 mb-4 mb-md-0 my-2 px-3" data-select2-id="46">
        <label for="select2Danger" class="form-label">Service</label> 
        <div class="select2-danger" data-select2-id="45">
            <div class="position-relative" data-select2-id="44">
                <select name="courses[]" id="select7Danger" class="select2 form-select select2-hidden-accessible" multiple="" data-select2-id="select7Danger" tabindex="-1" aria-hidden="true">
                    @foreach( $courses as $course )
                    <option value="{{$course->id}}" data-select2-id="courses{{$course->id}}">
                        {{$course->course_name}}
                    </option>
                    @endforeach
                </select>
    ...
    <!-- _________________________ -->
    <!-- _________________________ -->
    <div class="col-md-12 mb-4 mb-md-0 my-2 px-3" data-select2-id="40">
        <div class="select2-danger" data-select2-id="39">
            <div class="position-relative" data-select2-id="38">
                <select name="packages[]" id="select5Danger" class="select2 form-select select2-hidden-accessible" multiple="" data-select2-id="select5Danger" tabindex="-1" aria-hidden="true">
               
                    @foreach( $packages as $package )
                    <option value="{{$package->id}}" data-select2-id="package{{$package->id}}">
                        {{$package->name}}
                    </option>
                    @endforeach
                </select>
    ...
    <thead class="fs-7 text-gray-500 text-uppercase">
        <tr>
            ...
            <th class="min-w-150px sorting" tabindex="0" aria-controls="kt_profile_overview_table" rowspan="1" colspan="1" aria-label="Date: activate to sort column ascending" style="width: 205.188px;">Services</th>
    ...
    <tbody class="fs-6">
        ...
        <tr>
            ...
            <td>
                ...
                <br />
                @endforeach
                @foreach( $item->package as $element )
            ...
            <td>
                ...
                <div class="modal fade" id="Edit{{$item->id}}" tabindex="-1" aria-hidden="true" style="display: none;">
                    ...
                    <form method="POST" action="{{route('edit_promo', ['id'=>$item->id])}}">
                        ...
                        <!-- _________________________ -->
                        <div class="col-md-12 p-3" data-select2-id="64">
                          <label for="select2Dark" class="form-label">Services</label>
                          <div class="select2-dark" data-select2-id="65">
                            <div class="position-relative" data-select2-id="66">
                                ...
                                <option value="{{$course->course->id}}" selected="" data-select2-id="course2{{$course->course->id}}">{{$course->course->course_name}}</option>
                                @endforeach 

                                @foreach( $courses as $course )
                                <option value="{{$course->id}}" data-select2-id="course3{{$course->id}}">{{$course->course_name}}</option>
                                @endforeach
                            </select>
                            </div>
                          </div>
                        ...
                        <!-- _________________________ -->
                        <!-- _________________________ -->
                        <div class="col-md-12 p-3" data-select2-id="70">
                            <div class="select2-dark" data-select2-id="71">
                              <div class="position-relative" data-select2-id="72">
                                  <select name="packages[]" id="select4Dark" class="select2 form-select select2-hidden-accessible" multiple="" data-select2-id="select4Dark{{$item->id}}" tabindex="-1" aria-hidden="true">
                                 
                                  @foreach( $item->package as $package )
                                  <option value="{{$package->package->id}}" selected="" data-select2-id="package4{{$package->package->id}}">{{$package->package->name}}</option>
                                  @endforeach
  
                                  @foreach( $packages as $package )
                                  <option value="{{$package->id}}" data-select2-id="package5{{$package->id}}">{{$package->name}}</option>
                                  @endforeach
                              </select>
                              </div>
                            </div>
